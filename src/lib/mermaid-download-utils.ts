...
export class MermaidDownloadUtils {
  ...
  static async downloadMermaidDiagram(
    ...
    try {
      ...
      // Generate unique ID
      const diagramId = `mermaid-download-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      throw new Error(`Failed to download mermaid diagram: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private static processSvgForDownload(svg: string, options: MermaidDownloadOptions): string {
   ...
   * Download PNG file
   */
  private static async downloadPng(svg: string, filename: string, options: MermaidDownloadOptions): Promise<void> {
   ...
   * Download JPG file
   */
  private static async downloadJpg(svg: string, filename: string, options: MermaidDownloadOptions): Promise<void> {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
  ...
  static getDefaultFilename(mermaidCode: string): string {
    ...
    if (firstLine.startsWith('title')) {
      ...
      if (titleMatch) {
        return titleMatch[1].replace(/[^a-zA-Z0-9\s-]/g, '').trim().replace(/\s+/g, '-').toLowerCase();
      }
    }
